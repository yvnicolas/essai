...
public class DynCSVAddrBook extends DynExternalAddressBookImpl {
    ...
    private CellProcessor[] constructProcessors(String[] csvHeader) {
        ...
        ArrayList<CellProcessor> processorsAsList = new ArrayList<CellProcessor>(20);
        for (int i = 0; i < csvHeader.length; i++) {
     ...
     */
    private List<DynCSVContact> extractCSVContacts(ICsvBeanReader beanReader, String[] csvHeader) {

        List<DynCSVContact> contacts = new ArrayList<>();
        DynCSVContact contact;
        CellProcessor[] processors = constructProcessors(csvHeader);
        try {
            while ((contact = beanReader.read(DynCSVContact.class, csvHeader, processors)) != null) {
